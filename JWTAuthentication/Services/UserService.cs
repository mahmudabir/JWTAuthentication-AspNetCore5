...
namespace JWTAuthentication.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<string> AddRoleAsync(AddRoleModel model)
        {
            ...
            if (await _userManager.CheckPasswordAsync(user, model.Password))
            {
                var roleExists = Enum.GetNames(typeof(Roles)).Any(x => x.ToLower() == model.Role.ToString().ToLower());
                if (roleExists)
                {
                    var validRole = Enum.GetValues(typeof(Roles)).Cast<Roles>().Where(x => x.ToString().ToLower() == model.Role.ToString().ToLower()).FirstOrDefault();
